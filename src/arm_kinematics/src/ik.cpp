  ...
  std::optional<double> grasp_yaw)
{
  ...
  // 도달 불가로 표현
  //
  // ★ 이 가드에는 적혀 있지 않던 전제가 있다 (2026-08-07 벤치로 규명).
  //   "목표가 로봇 앞쪽 반공간에 있다" = cos(theta1) > 0.
  //   그 전제 위에서만 px = tip.r*cos(theta1) 의 부호가 tip.r의 부호와 같아져
  //   px < 0 이 뒤접힘과 동치가 된다 -> 오폭 0, 검출 100%.
  //   전제가 깨지면(뒤쪽 목표까지 허용하면) 가드는 정상 목표를 죽인다 — 실측 오폭 89.6%.
  //   실제 셀에서는 카메라가 앞만 보므로 전제가 자동으로 성립한다. bench_ik도 같은 정의역을 쓴다.
  if (px < 0.0) {
    sol.reachable = false;
    return sol;
  }
  ...
  auto t3 = get_elbow_angle(d, L1, L2, elbow_up);   // elbow_up 해당 위치까지 닿을 팔꿈치 각도
  auto t2 = get_shoulder_angle(wrist.r, wrist.z, L1, L2, elbow_up);   // 해당 목표 위치까지 어깨 각도
  ...
  // 이제 그리퍼 닫힘 축은 world의 grasp_yaw에 맞춘다.
  // joint5는 x축 roll인데 phi=-90에서 그 축이 아래를 향해 위에서 보면 반대로 돈다.
  sol.theta5 = grasp_yaw.has_value() ?
    std::remainder(sol.theta1 + M_PI_2 - grasp_yaw.value(), M_PI) :
    0.0;
  sol.reachable = true;
  return sol;
}
