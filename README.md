...
현재 로봇은 이런 사이클을 가지고 돌고 있다. 
사람의 명령 -> 파싱 및 실행 -> 시퀀스 완료
이렇게 순환해서 돌고 있다.

이제 구조를 다음과 같이 바꿔야 한다.
켠다 -> 씬을 본다. -> 물체가 있다. -> 집는다. -> 놓는다.
다시 씬을 본다 -> 물체가 있다 -> 집는다. -> 놓는다.

이것이 셀이다. 공장의 피킹 셀은 사람이 옆에서 하나씩 시키지 않는다. 켜두면 계속 알아서 일하고 잘못되면 스스로 수습하고 다시 일한다.

현재 계속 도는 로봇은 지금 뭘 하는 중인지 밖에서 알 수 있어야 한다. 안 그러면 멈춰 있는건지 생각중인지 구분이 안된다.
RUNNING : 정상적(관여 하지 않음), RECOVERING : 실패 후 수습 중(조금 더 지켜본다.), ABORTED : 멈춤(사람의 복구 필요)
핵심은 언제 사람을 다시 부르는 가에 있다.
그래서 할 일은 두가지 이다.
1. 끝지점을 시작으로 다시 잇는다. - 현재 시퀀스 완료에서 다시 처음 시작지점으로 보낸다.
2. 지금 무슨 상태인지 이름을 붙여 밖으로 보내 사람이 알 수 있게 만든다.

### 20일차

자가 명령 파악
: 물건이 어느 위치에 몇 초간 존재할 경우 명령을 내려야 하는 데 그 명령은 자연어인가 문자열 파싱인가

문자열 파싱으로 간다
문자열 파싱(SELF) -> 바로 스킬 서버에 명령어 보내 명령 수행
자연어(HUMAN) -> LLM -> 해석 가능한 문자열 -> 스킬 서버에 보내 명령수행

steps = 검증기가 아는 dict 리스트
plan = 실행 고리가 쓰는 (client, goal) 리스트

/command -> on_command -> _to_steps -> steps -> _distpatch -> validate_steps -> goals -> execute

작업 전체 타이틀
창고 - 작업 공간 - 하차
1. 작업 공간에 물건 이있으면 창고로 옮긴다.
2. 창고에서 pick을 해서 하차 장소로 옮긴다.

1에서 정리가 실패하면 _busy가 내려가고 블록은 계속 작업 구역에 남아있는 문제는 계속 시도하게 된다.
자가 정리 역시 다른 시도와 맞게 3회까지만 재 실행할 수 있다.
자가 정리 

RobotStatus라는 메시지를 만들어 따로 빼냈음

ABORTED_WAIT+ 후 재개
예상치 못한 종료가 일어났을 경우 알림을 띄우고 다시 재개한다.

### 21일차

3. 불량품제거 불량품은 초록색이며 블록과 크기와 집는 방법이 다르고 색만 다르다. 작업 공간이나 그런 곳에 있으면 bin으로 둬야 한다.

전체 물건을 둔 씬 전체를 생성후 테스트를 생성
1. 모서리를 무는 문제가 발생. 베이스각도 물체 yaw 조합을 두면서 체크 중

밖에서 얼마나 일했나가 보이게 한다.
RobotStatus에 4개 필드를 추가했다.

씬의 환경을 변경했다.

agent에서 씬의 상태를 파악해서 맞는지 행동이 제대로 끝났는지 점검한다.

현재 로봇이 상태를일어나다가 충돌이 일어나고 있다.

MoveIt은 자기가 아는 것만 피하고 아는게 로봇 뿐이다.
물체가 있다는것은 우리가 씬에서 알 고 있고 

1. 물체가 있다는 걸 아는 것 : arm_perception
2. 그 물체를 피해가는 각도 : MoveIt
3. 어디를 어떤 각도로 길게 묶을 것인가 : arm_skills
4. 관절각 : arm_kinematics
5. 집혔는지 판정 : arm_skill
6. 실패하면 무얼 할 것인가 : arm_agent
7. 애초에 무얼 할 것인가? : arm_agent

현재 ai가 씬에 없는 물체를 시켰는데 재프롬프트가 이상한 동작을 했다.
형식이 틀렸다에 쓰는 도구인데 재프롬프트가 명령을 바꿔치기해서 넣었다.

명령 red_block을 넣는데, green_block을 옮기라는 명령을 넣는다.

### 22일차
runpod 연결 기념 더 고급 모델 사용하여 테스트

현재 일어나는 문제들 정리
1. 바꿔치기 조용히 다른 물건을 옮기는 행동
초록 블록을 옮겨라. 초록 블록이 없을 경우 빨간 블록을 옮기는 명령을 llm이 바꿔서 내고 있음.
...
70.9% -> 76.9%    거부 경로를 염
76.9% -> 83.5%    거부 조건을 닫힌 목록으로 좁힘

중간 판에서 오거부가 20건 났다. "cannot be done"이라고 넓게 적었더니
모델이 존댓말, 대문자, 인사말까지 못 하는 일로 읽었다.
'red_block 집어' 3/3 거부, 'RED_BLOCK 집어', 'red_block 집어, 고마워', '홈으로 가' 전부 [].
영어 'pick red_block'은 통과했다.

거부 조건을 (a)씬에 없는 물체 (b)없는 target (c)없는 pose (d)계약 밖 동작으로 닫고
반대 의무를 넣었다. 씬에 있으면 반드시 계획을 내라. []는 항상 오답이다.
...
| 모델 | 점수 | 정확도 | 지연(중앙값) | GPU 적재 |
| --- | --- | --- | --- | --- |
| gemma4:26b | 162/182 | 89.0% | 4.42초 | 74% |
| exaone3.5:32b | 151/182 | 83.0% | 6.76초 | 74% |
| qwen3:8b | 147/182 | 80.8% | 1.13초 | 100% |
| exaone3.5:7.8b | 146/182 | 80.2% | 1.05초 | 100% |

v2 (2026-08-04, thinking 켠 채로 잰 값)

| 모델 | 점수 | 정확도 | 지연(중앙값) |
| --- | --- | --- | --- |
| exaone3.5:7.8b | 129/182 | 70.9% | 0.36초 |
| gemma4:e2b | 102/182 | 56.0% | 3.70초 |
| qwen3.5:9b | 96/182 | 52.7% | 12.54초 |
| llama3.1:8b | 59/182 | 32.4% | 0.82초 |

거부 92건 중 모델이 스스로 []를 낸 비율
gemma4:26b 86% / exaone3.5:7.8b 86% / exaone3.5:32b 80% / qwen3:8b 68%
옛날에는 0%였다. 거부 경로가 없어서 전부 검증기가 잡았다.

#### 2026-08-21 RunPod 전 모델 재측정 - 10개 언어 포함

현재 비교의 정본은 `v3-multilingual-242`다. 기존 182개 한·영 케이스를 그대로 앞에
두고, 한국어·영어·일본어·중국어·독일어·프랑스어·스페인어·이탈리아어·포르투갈어·
러시아어 60개를 뒤에 붙였다. RunPod RTX A4000 16GB, Ollama 0.32.14,
`think=false`, temperature 0.3, `k=3`이며 모델은 한 번에 하나만 적재했다. 한 케이스는
세 번의 계획 시도를 모두 맞혀야 통과한다(pass^3). 검증 실패 시 재질문이 한 번 더
발생할 수 있으므로 실제 LLM API 호출 수는 단순한 `242 x 3`보다 많을 수 있다.

위의 v2 표는 thinking을 켠 초기 기록, v3 표는 182개와 이전 프롬프트의 기록이다.
시험지와 프롬프트가 달라 세대 사이 점수를 직접 비교하지 않고, 아래 표 안에서만 모델을
비교한다.

##### 전체 242개 결과

| 순위 | 모델 | 점수 | 정확도 | 호출당 지연 중앙값 | 판정 |
| ---: | --- | ---: | ---: | ---: | --- |
| 1 | gemma4:26b | 220/242 | 90.9% | 3.57초 | 정확도 1위, 다국어 59/60 |
| 2 | qwen3.5:9b | 211/242 | 87.2% | 1.88초 | 정확도와 지연의 실용적 균형 |
| 3 | exaone3.5:7.8b | 191/242 | 78.9% | 1.14초 | 완주 모델 중 가장 빠름 |
| 4 | gemma4:12b | 186/242 | 76.9% | 2.95초 | 정상 명령은 강하지만 push를 전혀 거부하지 못함 |
| 5 | gemma4:e2b | 180/242 | 74.4% | 2.00초 | 정상 명령 대비 거부 성능이 약함 |
| 6 | llama3.1:8b | 101/242 | 41.7% | 1.93초 | 요청하지 않은 후속 동작을 자주 추가해 탈락 |
| DNF | exaone3.5:32b | 105/135 확인, 145에서 중단 | 77.8% (135개 구간) | 미산출 | 반복 30초 timeout, A4000에서 배포 부적합 |

`exaone3.5:32b`는 평가기가 종료 시점에만 CSV를 쓰기 때문에 최종 원본 파일이 없다.
135개까지 105개 통과는 확인했으며, 그 뒤 145번째까지 진행하다 중단했다. 135번째
시점부터 남은 문제를 모두 맞혀도 최대 212/242(87.6%)라 `gemma4:26b`를 이길 수
없고, 두 단계 계획은 30초 timeout과 재시도로 한 명령이 약 60초까지 걸렸다. 따라서
부분 점수를 완주 모델 순위에는 사용하지 않는다.

##### 기존 182개와 다국어 60개 분리

| 모델 | 기존 182개 | 중앙값 | 다국어 정상 | 다국어 거부 | 다국어 60개 | 중앙값 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| gemma4:26b | 161/182 (88.5%) | 3.58초 | 40/40 | 19/20 | 59/60 (98.3%) | 3.56초 |
| qwen3.5:9b | 155/182 (85.2%) | 1.86초 | 38/40 | 18/20 | 56/60 (93.3%) | 2.41초 |
| exaone3.5:7.8b | 145/182 (79.7%) | 1.11초 | 34/40 | 12/20 | 46/60 (76.7%) | 1.24초 |
| gemma4:12b | 136/182 (74.7%) | 2.95초 | 40/40 | 10/20 | 50/60 (83.3%) | 2.94초 |
| gemma4:e2b | 138/182 (75.8%) | 1.96초 | 38/40 | 4/20 | 42/60 (70.0%) | 2.01초 |
| llama3.1:8b | 89/182 (48.9%) | 1.90초 | 10/40 | 2/20 | 12/60 (20.0%) | 1.95초 |

##### 언어별 결과

언어당 6개(정상 4개, 거부 2개)의 작은 표본이다. 숫자는 해당 언어에서 세 번 모두
맞힌 시나리오 수다.

| 모델 | 한국어 | 영어 | 일본어 | 중국어 | 독일어 | 프랑스어 | 스페인어 | 이탈리아어 | 포르투갈어 | 러시아어 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| gemma4:26b | 6/6 | 6/6 | 6/6 | 6/6 | 6/6 | 6/6 | 6/6 | 5/6 | 6/6 | 6/6 |
| qwen3.5:9b | 6/6 | 6/6 | 4/6 | 6/6 | 5/6 | 6/6 | 6/6 | 6/6 | 5/6 | 6/6 |
| exaone3.5:7.8b | 6/6 | 6/6 | 3/6 | 5/6 | 5/6 | 4/6 | 5/6 | 4/6 | 4/6 | 4/6 |
| gemma4:12b | 5/6 | 5/6 | 5/6 | 5/6 | 5/6 | 5/6 | 5/6 | 5/6 | 5/6 | 5/6 |
| gemma4:e2b | 4/6 | 5/6 | 4/6 | 5/6 | 3/6 | 5/6 | 4/6 | 4/6 | 4/6 | 4/6 |
| llama3.1:8b | 0/6 | 2/6 | 2/6 | 3/6 | 0/6 | 2/6 | 1/6 | 0/6 | 1/6 | 1/6 |
| 완주 모델 합계 | 27/36 | 30/36 | 24/36 | 30/36 | 24/36 | 28/36 | 27/36 | 24/36 | 25/36 | 26/36 |

언어와 모델별 차이는 다음과 같다.

- `gemma4:26b`는 정상 40/40, 없는 물체 거부 10/10을 통과했고 이탈리아어 push만
  pick으로 바꿔 실행하려 했다.
- `qwen3.5:9b`는 일본어 home/push, 독일어 push, 포르투갈어 home이 약했지만 나머지
  56개는 통과했다. 26b보다 중앙값이 1.69초 짧아 기본 배포 후보로 가장 균형이 좋다.
- `gemma4:12b`는 10개 언어의 정상 명령 40/40과 없는 물체 거부 10/10은 통과했지만,
  push를 10개 언어 모두 다른 유효 동작으로 축소했다. 그래서 모든 언어가 똑같이 5/6이다.
- `exaone3.5:7.8b`는 한국어·영어가 6/6이고 가장 빨랐다. 일본어 pick/home과 여러
  유럽 언어의 push 거부가 약했다.
- `gemma4:e2b`는 정상 명령 38/40에 비해 없는 물체 거부 3/10, push 거부 1/10으로
  fail-closed 성능이 부족했다. 독일어에서는 red block과 blue ring pick도 실패했다.
- `llama3.1:8b`는 단순 pick 뒤 요청하지 않은 `place` 또는 `move_to`를 자주 추가했다.
  언어 문제가 아니라 계획 경계를 지키지 못하는 문제가 커서 사용하지 않는다.

##### 점수를 읽을 때의 제한

- 이 점수는 순수 모델 지능이 아니라 **모델 x 프롬프트 x JSON 스키마 x 검증기 x 문자열
  폴백** 전체의 결과다. 빈 scene 거부처럼 모델 출력이 틀려도 검증기가 막아 통과한 사례가
  있다.
- 기존 N2의 정답은 `place` 단독이지만, 현재 실물 정책은 물체 운반 명령을
  `pick -> place`로 실행하는 것이다. 이를 올바르게 확장한 출력도 기존 정답과 달라
  감점되므로 원점수를 임의로 보정하지 않았다.
- 기존 N6는 scene 정보가 `None`이어도 계획을 만드는 것이 정답이지만, 현재 실물 정책은
  scene이 없으면 취소하는 fail-closed다. 이 구간은 실제 안전성을 과대평가한다.
- `stop/멈춰` 행은 과거 계약 밖 명령 거부 시험이다. 실제 비상정지는 LLM이 아니라 로봇의
  결정론적 안전 계층에서 처리한다.
- 다국어는 언어당 6개뿐이므로 “10개 언어 안전 제어 완성”을 주장하지 않는다. 데모 입력
  범위이며, 실물 자율 실행은 허용한 명령 형태와 고정 scene에 한정한다.

결론은 정확도 우선이면 `gemma4:26b`, A4000에서 지연까지 고려한 기본 후보는
`qwen3.5:9b`, 속도 우선 fallback은 `exaone3.5:7.8b`다. 원본 응답과 케이스별 결과는
다음 CSV에 있다.

- [`gemma4:26b`](eval_results/eval_gemma4_26b_runpod-20260821-allmodels-k3.csv)
- [`qwen3.5:9b`](eval_results/eval_qwen3.5_9b_runpod-20260821-allmodels-k3.csv)
- [`exaone3.5:7.8b`](eval_results/eval_exaone3.5_7.8b_runpod-20260821-qc7kd5.csv)
- [`gemma4:12b`](eval_results/eval_gemma4_12b_runpod-20260821-allmodels-k3.csv)
- [`gemma4:e2b`](eval_results/eval_gemma4_e2b_runpod-20260821-allmodels-k3.csv)
- [`llama3.1:8b`](eval_results/eval_llama3.1_8b_runpod-20260821-allmodels-k3.csv)
...
1. K=0 가드 타이핑
실물 카메라는 캘리브레이션 파일이 없으면 k:[0.0, 0.0...]을 내고 있다. 
지금 코드는 self.fx is None으로만 막는다. 0.0은 None이 아니라 가드를 통과한다.

캘리브레이션 : 
카메라는 3D 세상을 2D 픽셀로 눌러 담는다. 캘리브레이션은 이 규익을 숫자로 알아낸다.

K란 
블록이 카메라에서 앞으로 50cm, 오른쪽으로 5cm 떨어진곳에 있을때 비율을 결정하는 숫자이다.
1. 얼마나 벗어났는가를 비율로 만든다. 화면에 거리 대비 5cm를 결정한다. 
2. 그 비율을 픽셀로 바꾼다.
3. 화면 중앙에서 그만큼 옮긴다.
fx  : 확대율
fy : 세로 확대율
cx, cy : 화면의 어디가 정면인가?
K는 카메라 자체의 성질이라서 어디에 있던지 바뀌지 않는다.

가제보에서는 fx값을 계산해서 보내줬는데 실물에선느 캘리브레이션 파일이 없으면 카메라가 K를 전부 0으로 보낸다. 그래서 재야 한다.
...
블록 0.187
링 0.006 
링은 hook에 가까운 동작이라 그리퍼의 닫힘만으로 물건이 집혔는지 파악을 할 수 없다.
...
관절별 최대속도의 5% 씩 가속도 설정 0.05(기본값 0.1)  

1. 컨트롤러 active 인지 확인
2. 관절 이름, 개수 순서가 모델과 맞는가
3. 현재 관절값이 한계 안이고 NaN이나 점프가 없는가
Moveit은 현재 자세에서 목표까지 경로를 짠다.
현재 자세가 실물 서보 한계에 돋라
joint2 -2.011 eeprom 한계 [1.868, +1.658]
joint3 +1.713 eeprom 한계 [-1.571, +1.675]
현재 팔이 중력으로 앉혀서 출발점이 한계 밖으로 도달한다.

home으로 명령을 보냈더니 한계밖 자세가 스스로 풀리고 명령으로 돌아온다. 

방향에 따라서 정확도가 다르다. 중력의 영향을 받기 때문이다.

가속도와 속도를 모두 두배로 올리고(0.1, 0.1) 재실행
home->init 8.25 -> 6.28
init->home 8.68 -> 6.53
시간이 24%만 줄었다.
속도만 2배면 시간이 절반에 가까워지는데 가속도가 같이 2배가 되면 궤적 모양 자체가 바뀌게 된다. 짧은 이동에서는 최고속도에 도달하지 못한채 가감속만 하는 구간이 생기고 그때 시간이 sqrt(2)배만 빨라지게 된다.
그래서 가속도의 크기를 더 높여야 한다.

따라서 가속도를 0.2로 증가했다.(0.1, 0.2)
home->init 8.25 -> 6.28 -> 5.53
init->home 8.68 -> 6.53 -> 6.78

한쪽(to_init)은 가속도가 다른 쪽은 속도(to_home)가 병목으로 쌓인다.
궤적이 대칭이 아니라 중력이 방향에 따라 다르게 작용하기 때문이다.


(0.2, 0.3)
home->init 8.25 -> 6.28 -> 5.53 -> 3.08
init->home 8.68 -> 6.53 -> 6.78 -> 5.15

(0.3, 0.45)
home->init 8.25 -> 6.28 -> 5.53 -> 3.08 -> 2.58
init->home 8.68 -> 6.53 -> 6.78 -> 5.15 -> 2.82

느리면 정확하다가 이 팔에서는 성립하지 않는다. 오히려 느리면 중력을 오랫동안 버티면서 간야 한다.

move_to 이동을 하게 되면 고정 된 값으로 끌려서 가게 된다. 해당 모터값을 유지한채로 끌려서 가게 된다.

### 25일차

파지 판정이 틀렸다. 못 집었는데 pick이 SUCCEEDED로 끝난다.

is_holding()은 gripper_joint_1 값이 임계(블록 0.050)보다 크면 "쥠"으로 본다.
sim에서는 맞았는데 실물에서 무너졌다.

  빈 손인데 0.1871이 나왔다. 임계의 3.7배다. 무조건 "쥠"이 된다.

왜 빈 손이 0이 아닌가.
gripper_controller는 position_controllers/GripperActionController인데
hardware_controller_manager.yaml에 joint 하나만 적혀 있고 나머지는 전부 기본값이다.
  allow_stalling           false
  stall_timeout            1.0
  stall_velocity_threshold 0.001
  goal_tolerance           0.01
  max_effort               0.0
close를 보내면 손가락이 막혀 속도가 죽는다. 1초 동안 0.001 아래면 stalled로 보고
allow_stalling이 false라 goal을 abort한다.
MoveIt 쪽에는 이게 TIMED_OUT으로 올라온다. 로그의 "그리퍼 close : TIMED_OUT"이 그것이다.

즉 실물 그리퍼는 빈 손 상태가 둘이다.
  끝까지 닫힌 빈 손    0.0015
  닫히다 만 빈 손      0.1871   <- 임계 하나로는 물었을 때(0.22)와 못 가른다
임계 0.050은 앞의 것만 상정하고 있었다. 뒤의 것이 나오면 무조건 "쥠"이다.

진짜 판별 신호는 관절값이 아니라 close의 결과다. 그리퍼만 직접 때려서 확인했다.
  빈 손    stalled=false  reached_goal=true    0.0015까지 닫힘
  물었을 때 stalled=true   reached_goal=false   0.2025에서 멈춤

★ 처음에 "abort하면 컨트롤러가 명령을 지금 위치로 바꿔 붙들어서 굳는다"고 적었는데
  틀렸다. 물고 있는 상태에서 사람이 블록을 빼내 보니 손가락이 -0.0015까지 따라 닫혔다.
  stall로 abort된 뒤에도 명령은 목표(0)에 남아 계속 밀고 있다는 뜻이다.
  그러면 0.1871에서 멈춰 있던 그 빈 손은 무엇이었나 - 아직 못 밝혔다.
  MoveIt이 TIMED_OUT으로 goal을 취소하면서 hold position이 걸린 것으로 의심하지만
  재현해서 확인하지 않았다. 확인 전까지는 "빈 손이 0이 아닐 수 있다"까지만 사실이다.

★ allow_stalling을 런타임에 true로 바꿔 봤지만 직후의 close가 여전히
  stalled=true reached_goal=false ABORTED로 끝났다. 이 파라미터는 선언만 동적이고
  실제로는 기동 시점 값을 쓰는 것으로 보인다. 바꾸려면 yaml에 적고 재기동해야 한다.

원래 코드는 에러 코드로 판정했었다. gripper.cpp 주석에 그 근거 체인이 남아 있다.
"stall은 sim 접촉 해석에 따라 안 날 수 있다"는 이유로 관절 위치로 바꿨는데 실물은 반대다.
실물에서는 stall이 확실하고 관절값이 못 믿을 값이다. 두 환경이 서로 다른 신호를 준다.

곁가지 - 이것 때문에 regrasp가 한 번도 안 돌았다.
STRATEGY는 GRASP_FAILED -> REGRASP인데 pick이 성공으로 보고되니 복구 계층에 실패가
도착하지 않는다. 판정이 먼저 고쳐져야 regrasp가 의미를 갖는다.
그리고 regrasp는 agent 계층 로직이라 ros2 action send_goal로 skill을 직접 부르면
어차피 안 탄다. agent 노드를 띄우고 명령을 거기로 줘야 한다.

쥐는 힘 자체는 위치 목표로 못 늘린다.
그리퍼 서보(dxl16)는 Operating Mode 5 = Current-based Position Control이다.
위치로 가되 토크가 Goal Current로 잘린다. 목표 0에 실제 0.22로 멈추고 TIMED_OUT이
뜨는 건 이미 전류 상한에서 밀고 있다는 뜻이라, 목표를 더 닫아도 힘은 안 는다.
늘리려면 Current Limit(주소 38, EEPROM) / Goal Current(주소 102, RAM)를 같이 올려야 하고
Goal Current는 Current Limit을 못 넘는다. 즉 600을 넘기려면 EEPROM을 건드려야 한다.
매 부팅마다 무조건 다시 쓰이고 설정을 되돌려도 한 번 더 부팅해야 서보가 원래 값으로
돌아오므로, 순정 툴로 팔을 쓸 때 값이 어긋난 채 남는다. 그래서 600으로 두었다.
