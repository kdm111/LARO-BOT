...
(389,323), camera_info fx=fy=554.38, cx=320, cy=240
...
u,v는 원점이 왼쪽 위다. 원점은 왼쪽 위 구석이고 cv2.moments로 뽑는 값이다.
...
원점에서 내 x축 방향으로 0.1245, 내 y축 방향으로 0.1497, z축 방향으로 1만큼 간 점
...
```
(389, 323) -> world (0.181, -0.069)   정답 (0.18, -0.07)   오차 1mm
(257, 305) -> world (0.202,  0.065)   정답 (0.20,  0.06)   오차 5mm
```

손으로 미리 계산한 값과 코드 출력이 소수 셋째 자리까지 같았다.
TF, 광축 규약, 내부 파라미터 셋 중 하나만 틀려도 이 숫자는 안 나온다. 그래서 이 일치가 배선이 맞다는 증거다.

**오늘의 버그 - 가드 하나가 네 값을 대표하지 않는다**

self.cs = msg.k[2] 로 오타를 냈다. self.cx는 \_\_init\_\_의 None 그대로였다.

```
if self.fx is None: return None
```

이 가드는 fx가 채워지면 열린다. 그런데 cx는 아직 None이었다. 그래서 u - self.cx 에서 TypeError로 죽었다.

증상이 헷갈렸다. 처음 몇 줄은 "변환 불가"가 정상적으로 찍히고(camera_info 도착 전이니까) 도착한 직후에 크래시했다.

어제 cv2.fintContours 오타와 같은 계열이다. 린터는 이름 오타를 못 잡는다.
한 콜백에서 값 여러 개를 채우면 가드도 대표값 하나로 두면 안 된다.

**린트**

E261 4건. 주석 앞 공백 2칸. 어제도 1위였고 오늘도 나왔다.
F841 1건. area 로그를 지웠는데 found 변수 계산만 남아 있었다.
