...
gripper_empty 감지
place의 transfer 구간에서 그리퍼 관절 q가 닫힘 쪽으로 떨어졌으면 운반 중 낙하로 판정하고 gripper_empty, stage::transfer로 abort.

상시 감시가 아니라 구간 끝에서 확인한다. 
감시는 move_to_pose가 끝난 직후마다 확인한다. 즉시 멈춰야 하는 일이 생기면 그때 바꾼다.
is_holding()은 쥐었다와 못 읽었다를 구분하지 못한다. 정착 대기가 2초 안에 실패하면 nullopt가 돌아오고 그것도 빈 손으로 읽힌다.
...
1. 쥔 채로 52초 두고 q를 기록한다. q가 왜 내려가는지
2. 쥔 블록을 강탈하고 place를 건다. 감지되는지

관측
정상 place는 완료됐다. 다만 TRANSFER q가 0.0619로 임계 0.05까지 여유가 0.012뿐이었다.
쥔 채로 두면 q = -0.00484 * t + 0.886 으로 내려간다. R^2 = 0.955.
그동안 블록은 lift 높이에 매달려 6.6mm 내려앉다가 52초째에 빠진다.
강탈하면 q가 0.0003이 되고 code 6, stage TRANSFER로 abort된다.

알게 된 것
q는 계단이 아니라 직선으로 내려온다. 손가락이 물체를 초당 0.0048씩 계속 파고든다.
...
관측
운반 중 파지 확인 q=0.0245로 빈 손 판정, code 6 / TRANSFER.
...
그리고 임계를 0.02로 낮추려던 판단이 틀렸다.
감지 시점 q가 0.0245였다. 0.02로 낮췄으면 못 잡았다.
낙하 후 q가 0으로 가는 데 시간이 걸리기 때문이다.
임계를 낮추면 잘못 잡는 건 줄지만 못 잡는 게 는다. 공짜가 아니었다.
